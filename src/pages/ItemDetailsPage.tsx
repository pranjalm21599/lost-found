          ...
          <div className="lg:col-span-5 bg-slate-100 dark:bg-slate-850 p-6 flex flex-col items-center justify-center border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800">
            {isImage && !imageLoadError ? (
              ...
              <div className="flex flex-col items-center text-center p-8 text-slate-400">
                <ImageIcon className="w-16 h-16 stroke-1 text-slate-300 dark:text-slate-600 mb-2" />
                <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">
                  Image Preview Unavailable
                </span>
                <span className="text-xs text-slate-400 mt-1">
                  {firstFile?.file_name || 'Uploaded image'}
                </span>
                {fileUrl && (
                  <a
                    href={fileUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-3 text-xs text-indigo-600 dark:text-indigo-400 hover:underline inline-flex items-center gap-1"
                  >
                    <ExternalLink className="w-3 h-3" />
